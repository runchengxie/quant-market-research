  ...
  return (
    <section className={`period-panel ${embedded ? "embedded" : ""}`}>
      <div className="period-heading">
        <div>
          <span className="section-kicker">跨市场小微盘流动性</span>
          <h3>按各市场最近可用数据比较小市值股票的流动性</h3>
          <p>
            各市场按最近可用数据划分市值区间，使用截至前一交易日的 20
            日平均成交额。数据更新不同步，各市场日期分别列出。
          </p>
        </div>
        <span className={`status-badge ${periodStatus}`}>{statusLabel}</span>
      </div>
      <ControlBar>
        <span className="control-label">时间区间</span>
        {options.map((value) => {
          const item =
            value === "latest"
              ? null
      ...
      <div className="period-meta">
        {selected
          ...
          : `各市场最近可用数据 · ${latestDates || "日期待补充"}`}
      </div>
      {selected && selected.status === "incomplete" && (
        <div className="callout compact">
          <span className="section-kicker">覆盖提醒</span>
          <p>
            该区间各市场的数据覆盖不同，下表展示已有数据，缺失市场保留为空。
          </p>
        </div>
      )}
      {selected && selected.status === "pending" && (
        <div className="callout compact">
          <span className="section-kicker">生成状态</span>
          <p>
            该区间的数据预计可以计算，网页尚未发布分组汇总。生成后会补充共同覆盖日期和市场明细。
          </p>
        </div>
      )}
      <div className="period-table">
        {available.length ? (
          <SimpleTable
            rows={available.flatMap((market) =>
              market.buckets.map((bucket) => ({
                market: `${market.market}（截至 ${selected ? (market.coverage_end ?? "日期待补") : (market.as_of ?? "日期待补")}）`,
                bucket: bucket.label,
          ...
          <p className="panel-note">当前时间段还没有可展示的分组汇总。</p>
        )}
      </div>
    </section>
  );
...
export function LiquidityPageLegacy() {
  ...
  return (
    <>
      <ThemeHeading
        kicker="小微盘历史研究 · 跨市场流动性"
        title="比较各市场小市值股票的成交规模"
        text="比较各市场小市值股票的流动性，并列出数据覆盖情况。页面仅展示汇总结果，原始行情不公开。"
      ...
      <section className="stat-grid">
        {markets.map((item, index) => (
          <Stat
            key={item.market}
            label={`${item.market}（市值低于1亿美元）`}
            value={`$${num(item.sub_100m_median_usd)}`}
            note={`${num(item.sub_100m_count)} 只股票`}
      ...
      <Panel
        title="小市值股票的成交额比较"
        tag={`${metricLabel} · ${logScale ? "对数" : "线性"}`}
      >
        <BarChart
          rows={comparison}
          labelKey="market"
      ...
      <Panel title="各市场市值分组明细" tag="美元成交额">
        <SimpleTable
          rows={bucketRows}
          columns={[
            ["market", "市场"],
      ...
      <div className="fine-print">
        <span className="section-kicker">数据与研究边界</span>
        <p>{readableNotes(summary.caveats)}</p>
      </div>
      <div className="research-grid">
        <article className="research-card">
          <span className="section-kicker">定义</span>
          <h3>可交易规模还受哪些因素影响</h3>
          <p>
            成交额反映市场交易规模。判断实际能买卖多少，还需查看此前的日均成交额、日成交额中位数、订单占市场成交的比例，以及交易对价格的影响。
          </p>
        </article>
        <article className="research-card">
          <span className="section-kicker">时间</span>
          <h3>使用前一交易日已知的数据</h3>
          <p>
            流动性指标使用前一交易日及更早的数据，确保判断依据在交易前已知。
          </p>
        </article>
        <article className="research-card">
          <span className="section-kicker">数据边界</span>
          <h3>日股数据待更新</h3>
          <p>
            日股数据已接入，仍需按相同统计口径更新本地数据，才能补充分组结果。
          </p>
        </article>
      </div>
    </>
