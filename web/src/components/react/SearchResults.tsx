...
export default function SearchResults({ initialQuery = '', base = '/' }: { initialQuery?: string; base?: string }) {
  ...
  useEffect(() => { fetch(withBase('/search-index.json', base)).then((response) => { if (!response.ok) throw new Error('搜索索引暂时不可用'); return response.json(); }).then(setRecords).catch((reason) => setError(String(reason.message ?? reason))); }, [base]);
