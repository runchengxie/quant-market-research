...
当时没有新增功能 helper 或源码字符串断言测试。研究摘要 HTML 内容和链接的验收已由回本研究任务完成。当时网页使用 Vite 单页入口，单页回退响应无法代替真实页面检查。当前网站已迁移到 Astro，研究页面有独立静态路由，新的直达、刷新和交互检查见 Pages 工作流及 `web/tests/e2e/`。
