...
def build_daily_portfolio_returns(connection, *, constituent_counts=(50, 100, 200, 400, 800)) -> pd.DataFrame:
    """Build equal-weight smallest-cap next-session returns with explicit missing marks.

    ``market_panel`` must expose date, symbol, market_cap, amount, adj_close,
    is_eligible and is_suspended. ``suspension_events`` identifies confirmed
    suspension dates with suspend_type == 'S'.
    """
    ...
    return connection.execute(f"""
        WITH dates AS (
            SELECT date, lead(date) OVER (ORDER BY date) AS return_date FROM (SELECT DISTINCT date FROM market_panel)
        ), ranked AS (
            SELECT p.date AS formation_date, d.return_date, p.symbol, p.adj_close AS formation_close,
            ...
            SELECT b.*, n.adj_close AS next_close,
                   coalesce(e.has_suspension, false) AS has_suspension
            FROM basket b LEFT JOIN market_panel n ON n.date=b.return_date AND n.symbol=b.symbol
            LEFT JOIN (SELECT DISTINCT symbol, date, true AS has_suspension FROM suspension_events WHERE suspend_type='S') e
              ON e.date=b.return_date AND e.symbol=b.symbol
        ), returns AS (
            SELECT *, CASE WHEN next_close>0 THEN next_close/formation_close-1 ELSE 0 END AS marked_return,
                   CASE WHEN (next_close IS NULL OR next_close<=0) AND NOT has_suspension THEN 1 ELSE 0 END AS unknown_mark,
                   CASE WHEN (next_close IS NULL OR next_close<=0) AND has_suspension THEN 1 ELSE 0 END AS stale_mark
            FROM marks
        )
        SELECT formation_date, return_date, constituent_count,
        ...
        FROM returns GROUP BY formation_date, return_date, constituent_count
